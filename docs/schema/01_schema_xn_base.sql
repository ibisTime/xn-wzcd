...
CREATE TABLE `tb_car_dealer` (
  ...
  `agreement_valid_date_start` DATETIME NULL COMMENT '合作协议有效期起',
  `agreement_valid_date_end` DATETIME NULL COMMENT '合作协议有效期止',
  ...
  PRIMARY KEY (`code`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8 COMMENT = '经销商信息表';
...
CREATE TABLE `tb_car_dealer_protocol` (
  ...
  PRIMARY KEY (`id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8 COMMENT = '经销商协议表';
...
CREATE TABLE `tb_insurance_company` (
  ...
  PRIMARY KEY (`code`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8 COMMENT = '保险公司信息表';
