...
public class TradeOrderAOImpl implements ITradeOrderAO {
    ...
    private void doTransferSell(TradeOrder tradeOrder) {
        ...
        if (refereeUser != null) {
            ...
            Double fenChengFee = this.sysConfigBO.getDoubleValue(SysConstants.FEN_CHENG_FEE);
            BigDecimal shouldPayFenCheng = tradeOrder.getFee().multiply(BigDecimal.valueOf(fenChengFee));

            //获取推荐人账户
            Account refereeUserAccount = this.accountBO.getAccountByUser(refereeUser.getUserId(),ECoin.ETH.getCode());
    ...
    private void doAmountCheck(Ads adsSell, BigDecimal tradePrice,
                               BigDecimal count, BigDecimal tradeAmount, String outOfLeftCountString) {
    ...
    public UserStatistics userStatisticsInfo(String userId) {
        ...
        UserStatistics userStatistics = this.tradeOrderBO.obtainUserStatistics(userId);
        //
        userStatistics.setBeiXinRenCount(this.userRelationBO.getRelationCount(userId));
        //
