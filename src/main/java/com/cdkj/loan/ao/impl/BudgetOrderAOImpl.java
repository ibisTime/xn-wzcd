...
public class BudgetOrderAOImpl implements IBudgetOrderAO {
    ...
    /*
     * @Override
     * @Transactional zOrder data = new BudgetOrder(); CreditUser
     * applyCreditUser = creditUserBO.getCreditUserByCreditCode(
     ...
     * data.setBankRate(StringValidater.toDouble(req.getBankRate())); Long
     * loanAmount = StringValidater.toLong(req.getLoanAmount()); Long
     * invoicePrice = StringValidater.toLong(req.getInvoicePrice());
     * data.setCompanyLoanCs((double) (loanAmount / invoicePrice));// 我司贷款成数
     * data.setIsAdvanceFund(req.getIsAdvanceFund()); Long fee =
     * StringValidater.toLong(req.getFee()); data.setGlobalRate((double) (fee /
     * loanAmount) + StringValidater.toDouble(req.getBankRate()));// 综合利率
     ...
     * data.setIsLicense(req.getIsLicense()); data.setLicense(req.getLicense());
     * data.setIsSiteProve(req.getIsSiteProve());
     * data.setSiteProve(req.getSiteProve());
     * data.setSiteArea(req.getSiteArea()); data.setCarType(req.getCarType());
     ...
     * data.setHkbMainPage(req.getHkbMainPage()); data.setGhHkb(req.getGhHkb());
     ...
     * data.setApplyDatetime(new Date()); EBudgetOrderNode node =
     * EBudgetOrderNode.START_NODE; data.setCurNodeCode(node.getCode()); if
     * (EButtonCode.SEND.getCode().equals(req.getDealType())) { node =
     * EBudgetOrderNode.getMap() .get(nodeFlowBO.getNodeFlowByCurrentNode(
     * EBudgetOrderNode.AREA_AUDIT.getCode()));
     * data.setCurNodeCode(EBudgetOrderNode.AREA_AUDIT.getCode()); } //
     * 使用gps更新列表 for (String gpsCode : req.getGpsList()) { Gps dataGps = new
     * Gps(); dataGps.setCode(gpsCode); gpsBO.refreshUseGps(dataGps); } //
     * 更新征信人员信息 for (XN632120ReqIncome reqIncome :
     * req.getCreditUserIncomeList()) { CreditUser creditUser = new
     * CreditUser(); creditUser.setCode(reqIncome.getCode());
     * creditUser.setMonthIncome(
     * StringValidater.toLong(reqIncome.getMonthIncome()));
     * creditUser.setSettleInterest(
     * StringValidater.toDouble(reqIncome.getSettleInterest())); creditUser
     ...
     * creditUserBO.refreshCreditUserIncome(creditUser); } String code =
     * budgetOrderBO.saveBudgetOrder(data); // 日志记录
     * sysBizLogBO.saveSYSBizLog(code, EBizLogType.BUDGET_ORDER, code,
     * node.getCode(), node.getValue(), req.getOperator()); return code; }
     */
    ...
    public void approveGlobalManager(String code, String operator,
        ...
        if (!EBudgetOrderNode.TWO_AUDIT.getCode().equals(
            budgetOrder.getCurNodeCode())) {
            throw new BizException(EBizErrorCode.DEFAULT.getCode(),
                "当前节点不是二审节点，不能操作");
        }
        ...
        if (EApproveResult.PASS.getCode().equals(approveResult)) {
            budgetOrder.setCurNodeCode(nodeFlowBO.getNodeFlowByCurrentNode(
                EBudgetOrderNode.TWO_AUDIT.getCode()).getNextNode());
        } else {
            budgetOrder.setCurNodeCode(nodeFlowBO.getNodeFlowByCurrentNode(
                EBudgetOrderNode.TWO_AUDIT.getCode()).getBackNode());
        }
